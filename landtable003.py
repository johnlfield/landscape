...
if attCSV != "":
    ...
    if os.path.exists(runtable): 
        ...
        with con:
            cur = con.cursor()
            cur.execute("CREATE TABLE runtable AS \
                        SELECT DISTINCT r.runno, r.fips, r.mukey, r.gridx, r.gridy, t.* \
                        FROM %s a \
                        JOIN %s r ON a.fips=r.fips AND a.gridx=r.gridx AND a.gridy=r.gridy AND a.mukey_int=r.mukey \
                        JOIN %s t \
                        JOIN %s c ON a.fips=c.fips \
                        WHERE (a.nlcd240_equiv=51 OR a.nlcd240_equiv=51) AND r.grazetill='graze' AND a.irr=0 AND r.irrig='N'" % (attCSV,archtab,treattab,counttab))
            rows = cur.fetchall()
