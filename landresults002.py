...
### upload runtable ##################
CSVtoDB("hugoton_attributes_runtable_pasture_test.csv", dbfile, "c")
...
with con:
    ...
    cur.execute("CREATE TABLE Summary AS \
                 SELECT m.Site AS Site, m.Treatment AS DC_run_id, AVG(m.DDC_yield) AS yield, AVG(m.dSOM) AS dSOM, AVG(n.N2Oflux) as N2O \
                 FROM Modeled m \
